...
#model_file = "../embeddings-data/vectors-phrase.bin"
...
def filter_array(processed, possibleTags):
    count = 0
    pos_count = len(processed['sentences'][0]['tokens'])
    #print 'pos: ' + str(processed['sentences'][0]['pos'])
    #print 'tokens: ' + str(processed['sentences'][0]['tokens'])
    res_arr = []
    while count < pos_count:
        pos_tag = processed['sentences'][0]['tokens'][count]['pos']
        lemma = processed['sentences'][0]['tokens'][count]['lemma']
        if pos_tag in possibleTags:
            res_arr.append(lemma)
        count = count + 1
    return res_arr        
